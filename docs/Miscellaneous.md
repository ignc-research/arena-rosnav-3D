...
# How to automating the scenario mode (for large scale benchmarking)
...
> __NOTE__: Since every robot model works slightly differently it is not possible to provide a comprehensive guide on how to include. Here we provide only some important steps, and tests to ensure your robot model is implemented properly
