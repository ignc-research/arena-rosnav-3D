...
bool GridMap::get_static_map()
{
  ...
  {
    ROS_INFO_STREAM("Is stacic map available: "
                    << "True");
    // set global_plan
...
void GridMap::raycastProcess()
{
  ...
  for (int i = 0; i < md_.proj_points_cnt; ++i)
  {
    ...
    if (!isInMap(pt_w))
    { // if not in map, select the nearest point in map & set occ=0
    ...
    { // if in map
      ...
      if (length > mp_.max_ray_length_)
      { // if in map, but exceed max_ray_length, set occ=0
        pt_w = (pt_w - md_.laser_pos_) / length * mp_.max_ray_length_ + md_.laser_pos_;
        pix_idx = setCacheOccupancy(pt_w, 0);
      }
      else
      { // if in map, and inside max_ray_length, set occ=1
...
int GridMap::setCacheOccupancy(Eigen::Vector2d pos, int occ)
{
  ...
  if (md_.count_hit_and_miss_[idx_ctns] == 2)
  { // 1, // occ probability update for hit less often
    md_.cache_pixel_.push(id);
  }
  else if (occ == 0)
  { // occ probability update for every miss
    md_.cache_pixel_.push(id);
  }
...
void GridMap::updateESDFCallback(const ros::TimerEvent & /*event*/)
{
  ...
  if (!md_.esdf_need_update_)
    return; // because of dynamic env, we need to update more often
...
void GridMap::updateESDF2d()
{
  ...
  {
    fillESDF([&](int y)
             { return md_.occupancy_buffer_inflate_[toAddress(x, y)] == 1 ? 0 : std::numeric_limits<double>::max(); },
             [&](int y, double val)
             { md_.tmp_buffer1_[toAddress(x, y)] = val; },
             min_esdf[1],
             max_esdf[1],
             1);
  }
  ...
  {
    fillESDF([&](int y)
             { return md_.occupancy_buffer_neg_[x * mp_.map_pixel_num_(1) + y] == 1 ? 0 : std::numeric_limits<double>::max(); },
             [&](int y, double val)
             { md_.tmp_buffer1_[toAddress(x, y)] = val; },
             min_esdf[1],
             max_esdf[1],
...
void GridMap::publishDynamicMap()
{
  ...
  for (int x = 0; x <= mp_.map_pixel_num_(0); ++x)
    for (int y = 0; y <= mp_.map_pixel_num_(1); ++y)
    {
      // if (md_.occupancy_buffer_[toAddress(x, y)] == 0) continue;
      int idx_inf = toAddress(x, y);
      if (md_.occupancy_buffer_dynamic_inflate_[idx_inf] == 0)
        continue;
