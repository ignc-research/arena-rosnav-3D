...
class recorder:
    def __init__(self) -> None:
        ...
        self.scenario = (
            rospy.get_param("scenario_file").replace(".json", "").replace("eval/", "")
        )
        ...
        if self.record_only_planner:
            with open(
                ...
                + "/{0}_{1}_{2}.csv".format(
                    self.local_planner, self.scenario, self.now
                ),
                "w+",
                newline="",
            ) as file:
            ...
            with open(
                self.dir_path
                + "/{0}_{1}_{2}_{3}.csv".format(
                    self.local_planner, self.waypoint_generator, self.scenario, self.now
                ),
                "w+",
                newline="",
            ) as file:
    ...
    def action_callback(
        self, msg_action: Twist
    ):  # variables will be written to csv whenever an action is published
    ...
    def addData(self, data: np.array):  # add new row to the csv file
        ...
        with open(
            self.dir_path
            + "/{0}_{1}_{2}.csv".format(self.local_planner, self.scenario, self.now),
            "a+",
            newline="",
        ) as file:
            writer = csv.writer(
