...
class sensor():
    ...
    def update_obstacle_odom(self):
        ...
        rospy.Subscriber("/pedsim_simulator/simulated_agents",
                         peds.AgentStates, self.cb_marker)
            ...
            if type(msg) == MarkerArray:
                ...
                if "dynamic" in topic:
                    if topic in self.obstacles_dyn:
                        ...
                        # update dyn obst
                        label = topic.replace(
                            "/flatland_server/debug/model/obstacle_dynamic_with_traj_", "")
                        label = int(label) + len(self.obst_topics_static) + 1
                    self.obstacles_dyn[topic] = [
                        pos, r, v, m.header.stamp, label]

                else:
                    if topic in self.obstacles_static:
                        # label = list(self.obstacles_static).index(topic)
                        label = topic.replace(
                            "/flatland_server/debug/model/obstacle_circle_static_", "")
                        label = int(label)
