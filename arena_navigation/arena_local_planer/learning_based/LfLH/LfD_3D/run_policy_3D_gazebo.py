...
bspline_update_dt = 1.0

# MPC params
collision_check_dt = 0.04
clearance = 0.01

knot_dt_coef_max = 1.5
...
class Predictor:
    ...
    def update_depth(self, msg):
        ...
        self.depth = np.asarray(cv_image).copy()
    ...
    def get_LfD_bspline(self):
        ...
        # print(len(np.where(np.isnan(self.depth))[0]) / 640.0 / 480.0, self.depth[~np.isnan(self.depth)].max(), self.depth[~np.isnan(self.depth)].min())
        ...
        with torch.no_grad():
            ...
            if plot_reconstruction:
                ...
                depth_4_plot[np.isnan(depth_4_plot)] = 3.0
...
if __name__ == '__main__':
    ...
    sub_goal = rospy.Subscriber("/move_base_simple/goal", PoseStamped, predictor.update_goal)
    sub_depth = rospy.Subscriber("/camera/depth/image_raw", Image, predictor.update_depth, queue_size=1)
    # sub_scan = rospy.Subscriber("/pcl_render_node/camera_pose", PoseStamped, predictor.update_depth_pose, queue_size=1)
    sub_scan = rospy.Subscriber("/hummingbird/pose", Odometry, predictor.update_odom, queue_size=1)
