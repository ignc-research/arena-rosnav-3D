...
class ManualTask(ABSTask):
    ...
    def __init__(self, ns: str, obstacles_manager: ObstaclesManager, robot_manager: RobotManager):
        ...
        # subscribe
        rospy.Subscriber(f'{self.ns}manual_goal',
...
class StagedRandomTask(RandomTask):
    def __init__(self, ns: str, obstacles_manager: ObstaclesManager, robot_manager: RobotManager, start_stage: int = 1, PATHS=None):
        ...
        assert os.path.isfile(
            self.json_file), "Found no 'hyperparameters.json' at %s" % json_file
    ...
    def reset(self):
        ...
        with self._map_lock:
            ...
            self.robot_manager.set_start_pos_goal_pos(
                Pose2D(
                    self.scenario.robotPosition[0], self.scenario.robotPosition[1], 0),
                Pose2D(self.scenario.robotGoal[0],
                       self.scenario.robotGoal[1], 0)
            )
            ...
            # todo: implement max number of repeats for scenario
...
def get_predefined_task(ns: str, mode="random", start_stage: int = 1, PATHS: dict = None):
    ...
    if mode == "random":
        ...
        # Register 3 agent clusters (1-3 agents per cluster)
        obstacles_manager.register_pedsim_agents(6)
        task = RandomTask(obstacles_manager, robot_manager)
        print("random tasks requested")
    if mode == "manual":
        rospy.set_param("/task_mode", "manual")
        obstacles_manager.register_pedsim_agents(3)
